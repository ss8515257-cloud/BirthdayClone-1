...
export default function Polaroid({ photo, index, onClick, className }) {
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={cn(
        'group relative block w-full rounded-sm bg-cream-white p-3 pb-14 shadow-premium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-pink',
        className
      )}
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-2 px-3 text-center">
        <p className="font-dancing text-lg leading-tight text-night-sky/80">
          {caption}
        </p>
        {date && (
